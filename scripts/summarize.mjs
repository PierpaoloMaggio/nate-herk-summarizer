...
const CHANNEL_ID = 'UC2ojq-nuP8ceeHqiroeKhBA';
...
async function summarize({ title, transcript }) {
  const systemPrompt = 'Sei un esperto analista tecnico specializzato in sviluppo assistito da AI (Claude Code, agenti, n8n, automazioni). Produci sempre output HTML pulito (solo <h2>, <h3>, <p>, <strong>, <ul>, <li>, <code>) senza tag <html>, <body> o <style>. Non aggiungere preamboli, commenti o note finali. Mai usare blocchi triple-backtick.';

  const userPrompt = `Ti fornisco la trascrizione automatica in inglese di un video di Nate Herk (canale "Nate Herk | AI Automation") intitolato: ${title}
...
<ul>
  <li>Da 4 a 8 punti, ciascuno una frase compatta su un concetto o una novità distinta affrontata nel video.</li>
</ul>

<h3>Step tecnici o tutorial</h3>
<p>Se il video contiene una demo o un tutorial, riporta in modo strutturato:</p>
<ul>
  <li><strong>Obiettivo della demo:</strong> cosa si vuole ottenere.</li>
  <li><strong>Comandi/config/funzionalità:</strong> elenco preciso di ciò che viene usato (es. comandi shell, file modificati, settings, hook, skill, MCP server).</li>
  <li><strong>Dettagli rilevanti:</strong> qualsiasi parametro, flag o configurazione importante.</li>
</ul>
<p>Se il video <strong>non</strong> contiene step tecnici, sostituisci questa sezione con: <em>Nessun tutorial pratico in questo video.</em></p>

<h3>Strumenti e funzionalità menzionati</h3>
<ul>
  <li><strong>Nome strumento:</strong> spiegazione breve in 1-2 frasi.</li>
</ul>

<h3>Vantaggi pratici</h3>
<p>Una sintesi in 2-4 frasi sui benefici concreti per chi usa Claude Code/n8n/agenti AI nel proprio workflow quotidiano.</p>

Tutto in italiano. Mantieni intatti nomi propri, sigle, riferimenti a strumenti e prodotti (Claude Code, n8n, Anthropic, OpenRouter, MCP, ecc.). Restituisci direttamente l'HTML pronto, senza markdown, commenti o blocchi di codice.
...
${transcript}`;
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'HTTP-Referer': 'https://github.com/PierpaoloMaggio/nate-herk-summarizer',
      'X-Title': 'Nate Herk Summarizer',
    },
    body: JSON.stringify({
...
<p style="color:#666;font-size:13px;margin:0 0 8px 0">Nuovo video &middot; Nate Herk | AI Automation</p>
  ...
  for (const entry of newOnes.reverse()) {
    ...
    try {
      ...
      if (transcript.length < MIN_TRANSCRIPT_LEN) {
        ...
        await sendEmail({ subject: `Nate Herk — ${entry.title}`, html });
